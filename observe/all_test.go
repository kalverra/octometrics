...
func TestGenerateAllObserveData_IgnoresNonJSONAndSkipsExisting(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(manifestPath, []byte(`{"type":"workflow_run","id":"555"}`), 0600))

	monitorPath := filepath.Join(wfDir, "octometrics-monitor-555.jsonl")
	require.NoError(t, os.WriteFile(monitorPath, []byte(`{"message":"cpu"}`), 0600))
	...
	require.NoError(t, os.WriteFile(outPath, sentinel, 0600))
